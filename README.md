...
Для тумблера, регулирующего температуру пола, создана разметка и индикатор рисуется при помощи svg path вычисляемых в js. Чтобы добавить возможность взаимодействия используется скрытый input range, нужно добавить aria атрибуты на html элемент его замещающий. Взаимодействие реализовано благодаря объекту Draggable из библиотеки greensock. В момент вращения вычисляется процент от пройденного сектора и изменяется значение input и меняется значени на тумблере и пересчитыывать кривые svg.
